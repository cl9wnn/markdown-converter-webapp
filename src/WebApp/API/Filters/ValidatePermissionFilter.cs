...
public class ValidatePermissionFilter(DocumentAccessService documentAccessService, RequiredAccessLevel requiredPermissionType): IAsyncActionFilter
{
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        if (permissionId < (int)requiredPermissionType)
        {
            context.Result = new ForbidResult();
            return;
        }
